return {
	deadlynumber = "\\#ff5555\\NÚMERO MORTAL: \\#dcdcdc\\",
	role = "Função: ",

	sheriffdied = "\\#ff0000\\O xerife foi assassinado!",
	murdererdied = "\\#00ffff\\O assassino morreu! Acabou!",

	infected = "\\#2eb249\\Você foi infectado!",

	simon = "\\#5555ff\\Simão\\#dcdcdc\\: ",
	simon_connected = "\\#5555ff\\Simão\\#dcdcdc\\ conectado",
	simon_eliminated = "\\#ff2828\\ foi eliminado pelo Simon!",

	-- Murder instructions
	murder_sheriff = "Como Xerife, você precisa encontrar o assassino e matá-lo. Se fizer isso corretamente, o jogo acaba e o assassino ganhará menos pontos. Mas se errar e acertar um inocente, você morre. Se morrer, você solta sua roupa.",

	murder_murderer = "Como Assassino, você precisa acertar pessoas para matá-las. Quando elas morrem, ganham menos pontos, mas tome cuidado com o Xerife, pois ele pode matar você. Se você morrer, ganha menos pontos.",

	murder_innocent = "Como Inocente, você precisa fugir do assassino. Você não sabe quem é, então não confie em ninguém. Se o Xerife morrer, você pode virar Xerife se pegar o coração.",

	-- Minigame descriptions
	desc_glass = "Atravesse a ponte sem cair! Apenas um vidro em cada linha é seguro. Você vai arriscar sua sorte ou deixar outra pessoa cair?",

	desc_lights_out = "Este é um jogo simples: sobreviva. Você ganha 10 pontos por sobreviver. Você pode atacar outros jogadores por pontos extras... mas será que vale o risco?",

	desc_elim_lights_out = "Este é um jogo simples: sobreviva. Não há nada que possa machucar você aqui, a menos que vocês comecem a atacar uns aos outros. Mas por que fariam isso?",

	desc_red_green_light = 'Chegue até a linha de chegada! Quando Toad gritar "Luz Vermelha!", não deixe ele ver você se mexendo! Você pode usar obstáculos para não ser visto. Tenha cuidado!',

	desc_mingle = "Fique no carrossel! Quando Waluigi chamar um número, entre em uma sala com EXATAMENTE aquela quantidade de jogadores. Use o interruptor para impedir que outros entrem. Cooperação é essencial!",

	desc_star_steal = "Pegue a Estrela e segure-a para aumentar sua pontuação! Acerte um jogador para roubar a Estrela dele! Você será eliminado se sua pontuação for baixa demais!",

	desc_bomb_tag = "Não fique segurando uma Bob-omb! Acerte outro jogador para passar sua Bob-omb para ele. Se estiver segurando uma quando o tempo acabar... você já sabe o que acontece.",

	desc_koth = "Chegue ao topo da colina! Fique no círculo para aumentar sua pontuação. Se sua pontuação for baixa quando o tempo acabar, você será eliminado!",

	desc_team_koth = "Chegue ao topo da colina! Fique dentro do círculo para aumentar sua pontuação. Você ganhará mais pontos se apenas a sua equipe estiver no círculo. Se sua pontuação estiver muito baixa quando o tempo acabar, você será eliminado! Mantenha sua posição!",

	desc_duel = "Derrote seu oponente! Tenha mais vida ou seja o último sobrevivente para ganhar um ponto. Faça 2 pontos para vencer o minijogo!",

	desc_team_duel = "É time contra time! Tenha mais vida ou seja o último sobrevivente para dar um ponto ao seu time. Faça 2 pontos para vencer!",

	desc_elim_duel = "Derrote seu oponente! Tenha mais vida ou seja o último sobrevivente para ganhar um ponto. Faça 2 pontos para vencer tudo!",

	desc_team_elim_duel = "É time contra time! Tenha mais vida ou seja o último sobrevivente para dar um ponto ao seu time. Faça 2 pontos para vencer tudo!",

	desc_elim_dice = "Pronto para testar sua sorte? Você tem 5% de chance de eliminar alguém ao acertá-lo, mas cada erro aumenta suas chances em 10%! Ser atingido aumenta suas chances em 5%. Quem sobreviverá?",

	desc_dice = "Pronto para testar sua sorte? Você tem 5% de chance de eliminar alguém ao acertá-lo, mas cada erro aumenta suas chances em 10%! Ser atingido aumenta suas chances em 5%. Seja o último sobrevivente!",

	desc_coinrain = "Moedas caem do céu! Pegue Moedas Amarelas para ganhar 1 ponto. Pegue Moedas Azuis para ganhar 5 pontos.",

	desc_death_hit = "O último jogador vivo vence! É bem simples. Se você for atingido, está ABSOLUTAMENTE perdido. Então não seja atingido.",

	desc_broken_lamp = "Apenas 1 jogador tem a lanterna. Acerte-o para roubá-la. Sem ela, a escuridão drena sua vida... Está bem escuro. Créditos ao @RetroGames.",

	desc_rope = "Aperte os botões A e Z rapidamente! Cada vez que puxar a corda, você ganha pontos. Quem conseguir a maior pontuação usando toda sua força vence!",

	desc_fiery = "Meteoros em chamas estão caindo do céu! Desvie deles ou você será queimado! Se perder toda sua vida, será eliminado e o fogo azul será permanente!",

	desc_murder = "Quem é o assassino? O minijogo possui três funções principais: Inocente, Xerife e Assassino. Os inocentes precisam sobreviver, o Xerife precisa matar o assassino e o assassino precisa matar todos!",

	desc_russian_roulette = "Escolha seu número favorito de 1 a 6 com o botão L! No final de cada rodada você puxa o gatilho. Se seu número for igual ao número mortal, você morre.",

	desc_run = "Corra até o topo! Existem moedas no final, pegue-as para ganhar pontos e gere Bob-ombs para atrapalhar seus oponentes. Se for atingido ou cair, você volta ao início!",

	desc_lava = "O chão é lava! Se ficar embaixo da lava, perderá vida constantemente. Fique em lugares altos e empurre seus oponentes!",

	desc_virus = "Existe um vírus perigoso! Se for infectado, começará a perder vida, mas pode acertar alguém para se livrar da infecção! Sobreviva o máximo possível.",

	desc_simon = "Compita contra seus amigos para realizar todos os comandos enviados no chat antes que o tempo acabe. Mantenha o foco, siga as regras e consiga a maior quantidade de pontos!",

	desc_bombthrower = "Bob-ombs estão caindo do céu! Se você perder toda a sua vida, perderá o minijogo! No final de cada rodada, moedas aparecerão no centro do cenário. Sobreviva o máximo de tempo possível!",

	desc_hot_ring = "Fique dentro do círculo! Se você sair dele, perderá vida. O círculo continua encolhendo, e você pode empurrar os outros para fora! Vença sendo o último sobrevivente!",

	minigame_text = "Minijogo ",

	spectate_text = "Assistir",

	music_text = "Música",

	spectate_warning = "\\#ff5050\\AVISO: Isso eliminará você! Pressione A novamente para continuar.",

	colorblind_text = "Modo para Daltônicos",

	ready_text = "Preparar!",

	waiting_text = "Esperando...",

	untilroundends = " até o fim da rodada",

	untilgameends = " até o jogo terminar",

	untilelimination = " até a eliminação",

	yourscore = "Sua pontuação: ",

	theirscore = "A pontuação deles: ",

	safescore = "Pontuação segura:",

	language_text = "Idioma",

	-- Itens de menu
	game_settings = "Configuração do Jogo",
	select_next_minigame = "Selecionar Próximo Minijogo",
	modifiers = "Modificadores",
	force_start_game = "Forçar Início do Jogo",
	cancel_game = "Cancelar Jogo",
	team = "Equipe",
	open_cs_menu = "Abrir Menu CS",
	personal_settings = "Configurações Pessoais",
	exit_menu = "Sair do Menu",
	coopdx_menu = "Menu CoopDX",

	-- Configurações do jogo
	game_mode_selection = "Seleção do Modo de Jogo",
	include_all_player_duel = "Incluir Duelo de Todos os Jogadores",
	total_minigames = "Total de Minijogos",
	final_duel = "Duelo Final",
	elimination_mode = "Modo de Eliminação",
	percent_ready_to_start = "Porcentagem Pronto para Começar",
	teams = "Equipes",
	team_selection = "Seleção de Equipe",
	select_teams = "Selecionar Equipes...",

	-- Nomes dos níveis
	toad_town = "Toad Town",
	koopa_keep = "Castelo Koopa",
	ds_fort = "Forte DS",
	duel = "Duelo",
	random = "Aleatório",

	-- Menu de modificadores
	super_speed = "Velocidade Super",
	high_gravity = "Gravidade Alta",
	low_gravity = "Gravidade Baixa",
	inverted_controls = "Controles Invertidos",
	instakill = "Morte Instantânea",
	z_button_challenge = "Desafio do Botão Z",
	b_button_challenge = "Desafio do Botão B",

	-- Mensagens
	starting_the_game = "Iniciando o jogo...",
	need_at_least_2_players = "Precisa de pelo menos 2 jogadores!",
	canceled_forced_start = "Início forçado cancelado.",
	are_you_sure = "Tem certeza? Pressione A novamente para continuar.",
	must_have_at_least_2_duelers = "Deve ter pelo menos 2 duelistas!",
	waiting_for_players = "Esperando os jogadores...",
	elimination_mode_text = "Modo de Eliminação",
	last_place_eliminated = "Penúltimo lugar eliminado",
	bottom_eliminated = "Fundo ",
	eliminated_suffix = " eliminado",
	confirm_duelers = "\\#50ff50\\Confirmar Duelistas",
	all_player_duel = "\\#ffff50\\Duelo de Todos os Jogadores",
	dueler = "Duelista ",
	total_duelers = "Total de Duelistas",
	on = "\\#50ff50\\Ativado",
	off = "\\#ff5050\\Desativado",
	mingle_only = "Apenas Mingle",
	choose = "Escolher",
	in_order = "Em Ordem",
	all = "Tudo",
	random_teams = "Aleatório",
	hosts_choice = "Escolha do Anfitrião",
	players_choice = "Escolha dos Jogadores",
	clean_install_notice = "AVISO:\nParece que você NÃO fez uma instalação limpa do Drench Game.\nPara reduzir os tempos de download, desinstale o mod e reinstale a versão mais recente.",

	-- Dicas
	hint_1 = "Wah-hah! Wario acha que você deveria socar seus oponentes para eliminá-los! Não tenha piedade!",
	hint_2 = "Sou eu, Mario! Você viu o pote de ouro? Isso deve ser um monte de moedas! Uau!",
	hint_3 = "Sou eu, Toad! Ouça, já joguei esses jogos antes! Se você tiver o Modo de Eliminação ativo... você desaparecerá para sempre se for eliminado! O horror! AAAAA!",
	hint_4 = "Olá, sou Luigi e estou aqui para lhe falar sobre o Modo de Escolha. O anfitrião pode escolher qualquer jogo que quiser jogar, incluindo configurar jogos de Duelo 1v1. Eu recomendo fortemente que você dê uma chance a essa opção.",
	hint_5 = 'Wah, aquele grande Toad em Luz Vermelha, Luz Verde é um TRAPACEIRO! Ele tentará te enganar e às vezes vai se virar ENQUANTO diz "Luz Verde"! Wah, apenas eu deveria ter permissão para trapacear!',
	hint_6 = "Toad de novo! Estamos lutando um contra o outro com essas moedas pairando sobre nós... deve ser uma metáfora de algo! Eu tenho certeza disso!",
	hint_7 = "O jogo favorito de Wario é Ponte de Vidro! NÃO há forma de dizer qual painel de vidro é seguro, mesmo se você trabalhou em uma fábrica de vidro! Eu sempre espero alguém ir primeiro para ver qual é o correto, wah ha!",
	hint_8 = "Este mod foi uma colaboração com muitas pessoas, muitas para listar nestas dicas! Mas a maior parte da programação foi feita por EmilyEmmi, que é muito legal. Essas dicas não são tendenciosas, é claro.",
	hint_9 = "Wah, eles dizem que não andar no carrossel em Mingle é 'trapacear', hein? Vou jogar meus oponentes para fora e metê-los em problemas! Que tal trapacear assim, hein?",
	hint_10 = "Roubar Estrela é um dos meus favoritos pessoais! Você se move mais lentamente enquanto segura a Super Estrela, então você precisará se esquivar, ha-ha!",
	hint_11 = "Sou Toad! Odeio jogar Pega-Bob-omb... sempre sou atingido no último segundo! Os jogadores que seguram Bob-ombs correm mais rápido, então meu conselho é tentar flanqueá-los! Gritar também ajuda! AAAAA!",
	hint_12 = 'Olá, sou Luigi e acho o minijogo "Rei da Colina" uma experiência agradável. Eu mesmo sou contra lutas, então apenas espero até a costa estar livre.',
	hint_13 = "Sou Wario! Quer esmagar seus inimigos em Duelo ou Luzes Apagadas? Use o Golpe de Terreno! Causa MUITO dano se você conseguir o tempo certo! Dê-me uma parte do dinheiro, já que foi meu invento!",
	hint_14 = "Todas as trilhas de música, exceto a de Mingle, foram criadas por murioz! Sem suas contribuições, você estaria ouvindo Bob-Omb Battlefield repetidas vezes...",
	hint_15 = 'Os mapas de Saguão, Mingle, Ponte de Vidro e Luz Vermelha, Luz Verde foram criados por biobak, que também está trabalhando no próximo hack de ROM "Retorno à Ilha de Yoshi"!',
	hint_16 = "Os mapas de Rei da Colina, Toad Town e Koopa Keep foram criados por Woissil com aviso prévio curto. A Fortaleza de Whomp foi usada como mapa de depuração, e você poderia cair e perder instantaneamente... não foi divertido.",
	hint_17 = "O mapa de Duelos foi criado por mim, EmilyEmmi. Por isso é tão simples comparado aos outros...",
	hint_18 = "SOU TOAD E ODEIO LUZES APAGADAS! Tento subir nas correntes para escapar, mas são muito escorregadias e caio e me machuco! AUCH!",
	hint_19 = "Wah, estive investigando Pega-Bob-omb e descobri que ESTÁ VICIADO! Eles sempre dão as Bob-ombs para os jogadores que as tiveram pelo menor tempo! Devem odiar vencedores como eu!",
	hint_20 = "Olá jogador. Você notou que algumas portas se recusam a abrir em Mingle? Aparentemente, menos portas estão disponíveis após a 3ª rodada. Espero que você ache este conselho útil.",
	hint_21 = "Você tem que cuidar de mais do que apenas jogadores em Roubar Estrela! Se cair em lava, você também perderá a Estrela! Tenha cuidado!",
	hint_22 = "Sou Wario e vou ganhar Duelos com minha informação exclusiva! Você pode obter uma cura completa se eliminar outro jogador. É a estratégia perfeita, já que sou o melhor lutador por aí!",
	hint_23 = "Este mod é trazido a você por nossos patrocinadores (totalmente legítimos) da comunidade YouTube do Squeex! Você pode ver seus anúncios no monitor no saguão.",
	hint_24 = "Esses arquivos de áudio costumavam ocupar 23,7 MB! Demorava uma eternidade para baixar. Depois que Squishy recortou e comprimiu todo o áudio, esse tamanho foi reduzido para apenas 2,5 MB. Uau...",

	-- HUD Playerlist translations
	unknown = "Desconhecido",
	player_num = "Jogador ",
	players_colon = "Jogadores:",
	name_header = "Nome",
	description_header = "Descrição",
	location_header = "Localização",
	ping_header = "Ping",
	game_wins_header = "Vitórias de Jogo\nVitórias de Minijogo",
	l_mods = "[L] Mods",
	l_players = "[L] Jogadores",
	more_suffix = " mais",
	no_mods_active = "Nenhum mod ativo",
	active_mods = "Mods Ativos (",

	-- HUD Main translations
	starting_in = "Começando em ",
	you_survived = "\\#50ff50\\VOCÊ SOBREVIVEU",
	you_died = "\\#ff2828\\VOCÊ MORREU",
	no_one_won = "\\#ff2828\\Ninguém ganhou...",
	multiple_winners = "\\#ffff50\\Múltiplos vencedores!",
	wins_suffix = "\\#ffff50\\ vence!",
	winners_colon = "\\#ffff50\\Vencedores:",
	alive = "\\#50ff50\\Vivo",
	dead = "\\#ff2828\\Morto",
	finished = "\\#50ff50\\Finalizado",
	spectator = "\\#c8c8c8\\Espectador",
	done = "Concluído",
	idle = "Inativo",
	ok = "OK!",
	not_ready = "Não pronto!",
	rankings = "\\#ffff50\\CLASSIFICAÇÃO",
	modifiers_title = "\\#ffff50\\MODIFICADORES",
	selected = "Selecionado ",

	-- Credits
	creator = "Criador",
	minigames = "Minijogos",
	ideas = "Ideias",

	-- Audio
	now_playing = "Tocando Agora",

	-- Spectator messages
	entered_spectator = "\\#ffff50\\Entrou no modo espectador.",
	exited_spectator = "\\#ffff50\\Saiu do modo espectador.",
	exit_spectator_after_game = "\\#ffff50\\Você sairá do modo espectador após este jogo.",
	exit_spectator_after_round = "\\#ffff50\\Você sairá do modo espectador após esta rodada.",
	exit_spectator_after_minigame = "\\#ffff50\\Você sairá do modo espectador após este minijogo.",

	-- Game messages
	made_spectator = "\\#ff5050\\Você foi tornado espectador. Mova-se novamente para cancelar.",
	connected = " \\#ffffff\\conectado.",
	can_rejoin = "\\#ffff50\\ pode entrar novamente para restaurar seu progresso.",
	progress_restored = "\\#ffff50\\Seu progresso foi restaurado!",
	moderator_pick = "\\#ffff50\\Como você é o primeiro moderador disponível, você escolherá este minijogo!",
	killed_player_heal = "\\#ffff50\\Você matou ",
	got_full_heal = "!\\n\\#ffff50\\Recebeu cura completa!",
	killed_player = "\\#ffff50\\Você matou ",
	killed_suffix = "!",
	correcting_desync = "Tentando corrigir dessincronização...",
	ready = "Pronto!",
	simon_jump = "Pule!",
	simon_attack = "Ataque!",
	simon_dont_move = "Não se mexa!",
	simon_walk = "Ande!",
	simon_lava = "Queime na lava!",
	simon_ledge = "Segure uma borda!",
	simon_run = "Corra!",
	simon_backflip = "Faça um backflip!",
	desc_freeze_tag = "Congele seus oponentes! Jogadores congelados podem ser descongelados por seus companheiros.",
	total_points_header = "Pontos Totais",
	l_next = "[L] Próximo",
	l_points = "[L] Pontos Totais",

	game_glass = "Ponte de Vidro",
	game_lights_out = "Luzes Apagadas",
	game_red_green_light = "Luz Vermelha, Luz Verde",
	game_mingle = "Mingle",
	game_star_steal = "Roubo de Estrela",
	game_bomb_tag = "Pega-Bob-omb",
	game_koth = "Rei da Colina",
	game_duel = "Duelo",
	game_dice = "Batalha de Dados",
	game_coin_rain = "Chuva de Moedas",
	game_death_hit = "Golpe Mortal",
	game_murder = "Mistério do Assassinato",
	game_fiery = "Chuva de Meteoros Flamejantes",
	game_balloon_madness = "Loucura dos Balões",
	game_hot_ring = "Anel Quente",
	game_freeze_tag = "Pega-Congelante",
	desc_balloon_madness = "Estoure os balões dos seus oponentes! Acerte os jogadores, estoure seus balões e seja o último sobrevivente!",
	glass_skip_warning = "\\#ff5050\\Você não pode pular painéis de vidro!",
	lights_out_points = "\\#ffff50\\+%d ponto(s) (%d DANO)",
	mingle_stay_on_carousel = "\\#ff5050\\Fique no carrossel!",
	tiebreaker_round = "Rodada de desempate!",
	slight_speed_boost = "\\#ffff50\\Você recebeu um pequeno aumento de velocidade!",
	balloon_popped_other = "\\#ffff50\\Estourou! %s ainda tem %d balão(ões)",
	balloon_popped_self = "\\#ff5050\\Seu balão estourou! (restam %d)",
	thawed_out = "\\#7ad3ff\\Você foi descongelado!",
	frozen_wait_for_teammate = "\\#ff5050\\Você foi congelado! Espere um colega descongelar você.",
	mercy_rule_eliminated = "\\#ff5050\\Eliminado pela regra de misericórdia-\nvocê não pode ganhar pontos suficientes para vencer.",
	points_distributed = "Os pontos de %s\\#ffff50\\ foram distribuídos entre %s\\#ffff50\\.",
	star_stolen_from = "%s\\#ffffff\\ roubou a \\#ffff50\\Estrela\\#ffffff\\ de %s\\#ffffff\\!",
	star_stolen = "%s\\#ffffff\\ roubou a \\#ffff50\\Estrela\\#ffffff\\!",
	simon_says_prefix = "\\#ffff50\\Simão diz: \\#ffffff\\",
	afk_warning = "\\#ff5050\\Você será forçado a virar espectador se não se mover em 10 segundos!",
	dice_roll_prefix = "%s\\#ffff50\\ rolou %d (precisava de %d+). ",
	dice_roll_kill = "RIP...",
	dice_roll_survived = "Você sobreviveu!",
	glass_points = "\\#ffff50\\+%d pontos",
	keep_moving_warning = "\\#ff5050\\Continue se movendo ou haverá consequências...",
	chance_to_kill = "\\#ff5050\\Chance de matar: (%d%%)",
	killed_by = " foi morto por ",
	no_loop_point = "Nenhum ponto de loop para testar...",
	no_moderator_permission = "\\#ff5050\\Você não tem permissão para usar este comando.\n(Você não é moderador)",
	download_link = "\\#00ffff\\Link para download: \\#ffffff\\",
	host_changed_control_scheme = "O anfitrião alterou o esquema de controles",
	spawned_object = "Objeto criado: \\#FFFF00\\%s\\#d5d5d5\\.",
	guest_deletion_disabled = "\\#ff4444\\A exclusão de objetos para convidados foi desativada pelo anfitrião!",
	deleted_nearest_object = "\\#ffff00\\Objeto mais próximo excluído",
	no_nearby_object = "Nenhum objeto próximo encontrado",
	all_spawned_objects_deleted = "\\#44ff44\\Todos os objetos criados foram excluídos!",
	only_host_clear_objects = "\\#ff4444\\Somente o anfitrião pode excluir todos os objetos!",
}
